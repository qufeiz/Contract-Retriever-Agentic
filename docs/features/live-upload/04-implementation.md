...
The golden screenshot for U-1 (the upload → cross-source cited answer) is captured against the **live demo** by driving a real upload + ask, and committed to this feature's `images/` directory (per the README ledger). The Playwright journey spec (`tests/journeys/live-upload.spec.ts`) asserts the visible outcome — the dropzone, the uploaded-file list, the cited cross-source answer, and the resolvable `[F:…]` chips — removable-handler-proof. The **content** is independently proven by `backend/eval/upload_run.py`, the stronger gate.
