...
> _Screenshot (capture pending — see the note in [04-implementation.md](04-implementation.md#screenshots--journey-suite-environment-note)): the assistant answering the contract-expiry question — 38 contracts, $18,924,883.79 combined value, cited rows, an honest "penalties not available" statement, and the agent trace showing `contracts.csv` opened._
...
> _Screenshot (capture pending): the agent trace panel listing the maps read and the files opened._
...
> _Screenshot (capture pending): the answer with the count, total, cited rows, and the green validation badge._
...
> _Screenshot (capture pending): a clicked citation chip resolving to the highlighted source row._
...
> _Screenshot (capture pending): the honest maintenance refusal with the $40,597.00 pivot._
...
> _Screenshot (capture pending): the filing-date conflict — both dates surfaced with both citations._
...
> _Screenshot (capture pending): the Carter judgment answered in Hebrew with identical $1,285 / Page-24 citation._
