...
| [upload-scope-substring-check-leaked-other-sessions.md](upload-scope-substring-check-leaked-other-sessions.md) | Widening read-scope from one root (`knowledge/`) to per-session roots broke isolation: the Bash scope check asked "does the command MENTION an allowed root?" (a substring), so a command that named its OWN session AND read ANOTHER session's dir was allowed — a cross-session data leak. Non-deterministic (model self-refused most runs → a false green on one manual check). Fix: verify EVERY path token is in scope. Locked by `test_hardening.py::test_bash_cannot_read_another_session_even_if_it_names_its_own` + the real-SDK `upload_run.py::isolation_gate` + the deny-enforcement positive control. Meta-lesson: a green isolation check that relies on the MODEL refusing is a false green — prove the sandbox denies. |
