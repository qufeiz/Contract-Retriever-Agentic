...
6. **Citation locator grammar is exact.** PDF with printed labels (the court file) → `#p<printed-PAGE-N>` (bounds-checked against parsed "📑 PAGE N" headers, NOT the 8 physical pages). PDF without labels (`case-story.pdf`) → `#p<physical-page>`. CSV → `#row=<Vendor>|<ISO-date>`, resolve-to-exactly-one-or-fail. → `docs/features/agentic-knowledge-assistant/02-examples.md`
